...
export function ChatWindow(props: {
  ...
  return (
    <>
    <div className={`flex flex-col items-center p-4 md:p-8 rounded grow overflow-hidden border h-96`}>
      <h2 className={`${messages.length > 0 ? "" : "hidden"} text-2xl`}>{emoji} {titleText}</h2>
      {messages.length === 0 ? emptyStateComponent : ""}
      <div
        className="flex flex-col-reverse w-full overflow-auto transition-[flex-grow] ease-in-out"
      ...
      <ToastContainer/>
          ...
          <form onSubmit={sendMessage} className="flex w-full flex-col h-10 relative mt-1">
          <div className="flex">
            {intemediateStepsToggle}
          </div>
          <div className="flex w-full mt-4 relative bottom-0">
            <input
              className="grow mr-8 ml-1 p-4 rounded"
              value={input}
